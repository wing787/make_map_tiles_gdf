...
def get_tile_coords_df(
    ...
    x_ne, y_ne = deg2num(lonlat_dict["max_lat"], lonlat_dict["max_lon"], zoom_level)  # 右上
...
def num_to_tile_bounds(xtile: int, ytile: int, zoom: int) -> tuple[float, float, float, float]:
    """
...
def main(read_gdf: gpd.GeoDataFrame, zoom_level: int = None, output_dir: str = None) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate map tiles for Any vector file.")
    parser.add_argument(
        "--vector-filepath",
        required=True,
        type=str,
        help="タイル座標を生成するベクターファイルのパス"
    )
    parser.add_argument(
        "--zoom-level",
        required=True,
